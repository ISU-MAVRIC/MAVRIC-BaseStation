...
const max_steer_angle =
  Math.atan(wheel_length / wheel_width) - (10 * Math.PI) / 180;
...
export const calculateDriveValues = (
  DRIVE_STATE,
  tdrive,
  tsteer,
  cdrive,
  ...
  if (DRIVE_STATE == DRIVE_STATES.TANK) {
    ...
    let str_r = Math.sqrt(
      Math.pow(wheel_width / 2, 2) + Math.pow(wheel_length / 2, 2),
    );
