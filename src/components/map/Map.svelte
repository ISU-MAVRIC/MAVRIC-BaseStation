<script>
	...
	*/
	...
	function createMap(container) {
		...
		let southWestBound = L.latLng(
			DEFAULTS.MAP.SOUTHWEST_BOUND[0],
			DEFAULTS.MAP.SOUTHWEST_BOUND[1],
		);
		let northEastBound = L.latLng(
			DEFAULTS.MAP.NORTHEAST_BOUND[0],
			DEFAULTS.MAP.NORTHEAST_BOUND[1],
		);
	...
	function roverIcon(rotation) {
		let html = `<div class="map-marker"><div style="width: 20px; height:20px; transform-origin: center; transform: rotate(${rotation}deg);" >${roverIcons.library}</div></div>`;
	...
	function createLines() {
		return L.polyline([roverCoords].concat(markerLocations), {
			color: "#FF0000",
			opacity: 0.2,
		});
	}
